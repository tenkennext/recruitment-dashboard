...
export default function App() {
  ...
  const loadJobRequirements = async () => {
    try {
      const requirements = await getJobRequirements();
      setJobRequirements(requirements);
    } catch (err) {
      setError('No se pudo cargar los requerimientos.');
    }
  };
  ...
  const loadCandidates = async () => {
    ...
    try {
      const data = await getCandidates({ q: searchTerm, verdict: filterVerdict });
      setCandidates(data);
    } catch (err) {
      setError('No se pudo cargar los candidatos.');
      setCandidates([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    loadCandidates();
  }, [searchTerm, filterVerdict]);

  const filteredCandidates = candidates;
  const top3 = useMemo(
    () => candidates.filter((candidate) => candidate.isTop3).sort((a, b) => b.score - a.score),
    [candidates]
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {error ? (
          <div className="bg-rose-50 text-rose-700 border border-rose-200 rounded-2xl p-6 mb-8">
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'overview' && (
            ...
            >
              <section>
                ...
                {loading && !jobRequirements.length ? (
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-500">Cargando requerimientos...</div>
                ) : (
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {top3.map((candidate, idx) => (
                    ...
                    >
                      ...
                      <p className="text-sm text-slate-600 mb-4 line-clamp-3 italic">"{candidate.strengths}"</p>
                      <button
                        onClick={() => {
              ...
              <section className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">Total Candidatos</p>
                  </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-slate-900">{candidates.filter((c) => c.verdict === 'Avanzar').length}</p>
                    <p className="text-xs text-slate-500 font-medium">Avanzan</p>
                  </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-slate-900">{candidates.filter((c) => c.verdict === 'Considerar').length}</p>
                    <p className="text-xs text-slate-500 font-medium">Considerar</p>
                  </div>
                </div>
                <div className="bg-white p-4 rounded-xl border border-slate-200 flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-slate-900">{candidates.filter((c) => c.verdict === 'Descartar').length}</p>
          ...
          {activeTab === 'candidates' && (
            ...
            >
              ...
              {loading ? (
                <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-500">Cargando candidatos...</div>
              ) : (
      ...
      <footer className="bg-white border-t border-slate-200 py-8 mt-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="text-sm text-slate-500">Informe de Screening Basado en Evidencia • {new Date().toLocaleDateString()}</p>
